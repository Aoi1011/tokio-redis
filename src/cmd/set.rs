...
///
/// * EX `seconds` -- Set the specified expire time, in secnds.
...
impl Set {
    ...
    /// Parse a `Set` isntance from a received frame.
    ///
    ...
    pub(crate) fn parse_frames(parse: &mut Parse) -> crate::Result<Set> {
        ...
        match parse.next_string() {
            ...
            Ok(s) if s.to_uppercase() == "PX" => {
                // An expiration is specified in milliseconds. The next value is an
                // integer.
