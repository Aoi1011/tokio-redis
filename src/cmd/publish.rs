...
impl Publish {
    ...
    /// Parse a `Publish` isntance from a received frame.
    ///
    ...
    ///
    ///  The response is written to `dst`. This is called by the server in order to execute a
    ///  received command.
    pub(crate) async fn apply(self, db: &Db, dst: &mut Connection) -> crate::Result<()> {
        ...
        // message. Given this, `num_subscribers` should only be used as a "hint".
        let num_subscribers = db.publish(&self.channel, self.message);

        // The number of subscribers is returned as the response to the publish request.
