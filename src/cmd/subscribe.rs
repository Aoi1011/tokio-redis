...
impl Subscribe {
    ...
    ///  channels to subscribe to. Additional `subscribe` and `usubscribe` commands may be received
    ///  from the client and the list of subscriptions are updated accordingly.
    ///
    ...
    pub(crate) async fn apply(
        ...
        // Each individual channel subscription is handled using a `sync::broadcast` channel.
        // Messages are then focused out to all clients currently subscribed to the channels.
        //
        // An individual client may subscribe to multiple channels and may dynamically add and
        // remove channels from its subscription set. To handle this, a `StreamMap` is used to
        // track active subscriptions. The `StreamMap` merges messages from individual broadcast
        // channels as they are received.
        let mut subscriptions = StreamMap::new();

        loop {
            // `self.channels` is used to track additional channels to subscribe to. When new
            // `SUBSCRIBE` commands are received during the execution of `apply`, the new channels
            // are pushed onto this vec.
...
async fn handle_command(
    ...
    // A comand has been received from the client.
    //
    ...
    match Command::from_frame(frame)? {
        ...
        Command::Unsubscribe(mut unsubscribe) => {
            // If no channels are specified, this requests unsubscribing from **all** channels. To
            // implement this, the `unsubscribe.channels` vec is populated with the list of
...
///
/// All of these functions take the `channel_name` as a `String` instead of a  `&str` since
/// `Bytes::from` can reuse the allocation in the `String`, and taking a `&str` would require
/// copying the data. This allows the caller to decide whether to clone the channel name or not.
...
/// Creates the reponse to an unsubscribe request.
...
/// Creates a message informing the client about a new message on a channel that the client
/// subscribe to.
...
impl Unsubscribe {
    ...
    /// Parse an `Unsbuscribe` instance from a received frame.
    ///
    /// The `Parse` argement provides a cursor-like API to read fields from the
    /// `Frame`. At this point, the entire frame has already been received from the socket.
    ///
    ...
    /// # Returns
    ///
    /// On success, the `Unsubscribe` value is returned. If the frame is malformed, `Err` is
    /// returned.
    ///
    /// # Format
    ///
    ...
    pub(crate) fn parse_frames(parse: &mut Parse) -> Result<Unsubscribe, ParseError> {
        ...
        // Once all values in the frame have been consumed, the command is fully parsed.
        loop {
            match parse.next_string() {
                // A string has been consumed from the `parse`, push it into the list of channels
                // to unsubscribe from.
                Ok(s) => channels.push(s),
